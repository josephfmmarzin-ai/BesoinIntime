...
**Manuelle** : télécharger `BesoinIntime-v3.4.1.zip` dans la [dernière release](../../releases/latest), dézipper, et copier le dossier `BesoinIntime` dans `C:\Users\<toi>\Zomboid\mods\`. Le dossier doit contenir `mod.info`, `42/` et `common/`. Activer *Besoin Intime* dans le Mod Manager. En multijoueur, le serveur doit aussi avoir le mod.
...
**Manual**: download `BesoinIntime-v3.4.1.zip` from the [latest release](../../releases/latest), unzip, and copy the `BesoinIntime` folder into `C:\Users\<you>\Zomboid\mods\`. The folder must contain `mod.info`, `42/` and `common/`. Enable *Besoin Intime* in the Mod Manager. In multiplayer the server needs the mod too.
...
## Changelog
- **3.4.1** — Voice clips never repeat twice in a row (random among the others) and never overlap (minimum spacing longer than the longest clip).
- **3.4.0** — Removed the bed-creak loop and the relief chime. The three voice clips are now the only sounds: declared as three separate sounds, one picked at random in Lua every 2–5 s during the moment and once at the end. Diagnostic line `[BesoinIntime] voice play:` in console.
- **3.3.0** — Three voice clips played at random (one picked at random, random 2–5 s intervals) during the moment, heard by nearby players. Optional full black screen for the local player during the action (opacity tunable).
- **3.2.1** — Stats now use the Build 42 `CharacterStat` API (`stats:get/set(CharacterStat.STRESS)` etc.), with fallback to the old accessors. Before this, on 42.13+ the action played but did not change stress/unhappiness/boredom/fatigue.
- **3.2.0** — All stat accessors are now guarded (Build 42 removed some setters; no more errors at the end of the action). Accents decoded at runtime via `string.char`. Censor box raised to the pelvis (64 px default). Looping sound replaced by a rhythmic bed-spring creak with muffled breathing; replace `42/media/sound/BesoinIntime_moment.ogg` with your own file if you prefer.
